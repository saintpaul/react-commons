...
var lodashChain = require("lodash/chain");
...
CallAjax.uploadFile = function (url, data, onProgress) {
    return CallAjax._configure("POST", url, data, { processData: false, onProgress: onProgress });
};
...
CallAjax.showSpinner = function () {
    if (!CallAjax.disableSpinner) Configuration.showSpinner();
};
...
function Batch(callAjaxList) {
    ...
    this.processQueue = function () {
        ...
        var queries = lodashMap(next, function (query) {
            return query();
        });
        ...
        return $.when.apply($, _toConsumableArray(queries)).done(function () {
            ...
            if (queries.length === 1) _this.succeededCalls.push(results[0]);else lodashChain(results).filter(function (r) {
                return r;
            }).map(function (r) {
